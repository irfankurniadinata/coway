package com.example.coway.utils
...
class DBHelper(context: Context, factory: SQLiteDatabase.CursorFactory? = null) : SQLiteOpenHelper(context, DatabaseName, factory, DatabaseVersion) {
    ...
    fun getCustomer(): List<User> {
        val db = this.writableDatabase
        val selectQuery = "SELECT * FROM $TableName"
        val users: MutableList<User> = mutableListOf()
        val user = db.rawQuery(selectQuery, null)
        if (user != null && user.moveToFirst()) {
            do {
                users.add(User(name = user.getString(user.getColumnIndexOrThrow(columnName)), email = user.getString(user.getColumnIndexOrThrow(columnEmail))))

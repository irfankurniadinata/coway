package com.example.coway.ui.custom
...
import com.example.coway.utils.convertDpToPixel
import com.example.coway.utils.gone
import com.example.coway.utils.setTextHtml
import com.example.coway.utils.visible

class CowayToast(private val context: Activity?) {
    ...
    init {
        val popupView: View =
            LayoutInflater.from(context).inflate(R.layout.custom_coway_toast, null)
    ...
    companion object {
        ...
        fun make(activity: Activity, message: String, duration: Long) : CowayToast{
            return CowayToast(activity).setMessage(message).setDismiss(duration)
        }
    }
    ...
    fun setDismiss(duration: Long) : CowayToast{
        setDismiss(duration, null)
        return this
    }

    val handler = Handler()
    fun setDismiss(duration: Long, callback: Callback?): CowayToast {
        if (duration > 0) {
    ...
    fun setMessage(message: String): CowayToast {
        tvMessage.text = message
        return this
    }
